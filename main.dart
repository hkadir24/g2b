...
class SplashScreen extends StatelessWidget {
    ...
    if (snapshot.exists) {
      ...
      for (var m in snapshot.children.toList()) {
        Game new_game = Game(
          ...
          image: m.child("image").value.toString(),
        );
  ...
  Future getbyidFirebaseAllOrderHistory() async {
    final snapshot = await ref.child('orderHistories').get();
    if (snapshot.exists) {
      for (var m in snapshot.children.toList().reversed) {
        for (var k in m.children) {
          if (k != null) {
            print(k.value);
            Delivery new_game = Delivery(
              ...
              address: k.child("address").value.toString(),
            );
            delist.add(new_game);
          }
        }
  ...
  Widget build(BuildContext context) {
    ...
    getbyidFirebaseAllOrderHistory();
